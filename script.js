...
// ---------- MAP DATA (7 maps following chronology) ----------
const MAPS = [
  // 0 - Menteng 31
  {
    id: 'menteng',
    title: 'Menteng 31 · 15 Agustus Malam',
    objective: 'Bicara dengan para pemuda. Kumpulkan informasi, lalu keluar menuju Pegangsaan.',
    bg: '#0c1210',
    spawn: { x: 80, y: 350 },
    walls: [
      { x: 0, y: 0, w: 800, h: 20 }, { x: 0, y: 460, w: 800, h: 20 },
      { x: 0, y: 0, w: 20, h: 480 }, { x: 780, y: 0, w: 20, h: 480 },
      { x: 300, y: 100, w: 200, h: 20 }, { x: 300, y: 100, w: 20, h: 120 }
    ],
    hides: [],
    npcs: [
      { x: 200, y: 200, name: 'Sukarni', r: 14, color: '#eab308',
        dialog: ['Kita harus bergerak malam ini juga.', 'Soekarno masih ragu. Kita bawa mereka ke tempat aman.', 'Pergi ke Pegangsaan Timur. Jangan sampai ketahuan Jepang.'] },
      { x: 400, y: 280, name: 'Wikana', r: 14, color: '#38bdf8',
        dialog: ['PPKI buatan Jepang tidak bisa dipercaya.', 'Proklamasi harus dari rakyat, bukan dari mereka.', 'Hati-hati di jalan.'] },
      { x: 550, y: 180, name: 'Chaerul Saleh', r: 14, color: '#f472b6',
        dialog: ['Kalau perlu kita paksa.', 'Semangat pemuda tidak boleh padam!'] }
    ],
    items: [
      { x: 150, y: 120, id: 'surat_menteng', name: 'Catatan Rapat', icon: '📜', taken: false }
    ],
    patrols: [],
    exits: [
      { x: 720, y: 300, w: 50, h: 60, to: 1, label: 'Ke Pegangsaan →' }
    ],
    alarmEnabled: false
  },

  // 1 - Jalan ke Pegangsaan
  {
    id: 'jalan',
    title: 'Jalan Menuju Pegangsaan · Malam',
    objective: 'Sampai ke rumah Soekarno. Hindari patroli Jepang!',
    bg: '#0a100a',
    spawn: { x: 50, y: 240 },
    walls: [
      { x: 0, y: 0, w: 800, h: 40 }, { x: 0, y: 440, w: 800, h: 40 },
      { x: 250, y: 80, w: 30, h: 150 }, { x: 450, y: 250, w: 30, h: 180 }
    ],
    hides: [
      { x: 100, y: 80, w: 70, h: 50 }, { x: 300, y: 300, w: 70, h: 50 },
      { x: 500, y: 100, w: 65, h: 50 }, { x: 620, y: 320, w: 70, h: 50 }
    ],
    npcs: [],
    items: [
      { x: 380, y: 150, id: 'kunci_pagar', name: 'Kunci Pagar', icon: '🔑', taken: false }
    ],
    patrols: [
    ...
    exits: [
      { x: 730, y: 200, w: 50, h: 60, to: 2, label: 'Rumah Soekarno →' }
    ],
    alarmEnabled: true
  },

  // 2 - Depan Rumah Soekarno
  {
    id: 'depan_rumah',
    title: 'Pegangsaan Timur 56 · Depan Rumah',
    objective: 'Masuk ke rumah tanpa ketahuan. Gunakan semak untuk sembunyi.',
    bg: '#0b130b',
    spawn: { x: 60, y: 300 },
    walls: [
      { x: 500, y: 0, w: 30, h: 180 }, { x: 500, y: 250, w: 30, h: 230 },
      { x: 650, y: 80, w: 130, h: 20 }, { x: 650, y: 80, w: 20, h: 200 }
    ],
    hides: [
      { x: 120, y: 60, w: 70, h: 55 }, { x: 200, y: 280, w: 75, h: 55 },
      { x: 350, y: 100, w: 70, h: 50 }, { x: 380, y: 350, w: 70, h: 50 }
    ],
    npcs: [],
    items: [],
    patrols: [
    ...
    exits: [
      { x: 680, y: 200, w: 50, h: 55, to: 3, label: 'Masuk Rumah 🚪' }
    ],
    alarmEnabled: true
  },

  // 3 - Dalam Rumah Soekarno
  {
    id: 'dalam_rumah',
    title: 'Dalam Rumah Soekarno · 22.00 WIB',
    objective: 'Bicara dengan Soekarno & Hatta. Yakinkan mereka ikut.',
    bg: '#1a120c',
    spawn: { x: 100, y: 350 },
    walls: [
      { x: 0, y: 0, w: 800, h: 30 }, { x: 0, y: 450, w: 800, h: 30 },
      { x: 0, y: 0, w: 30, h: 480 }, { x: 770, y: 0, w: 30, h: 480 },
      { x: 350, y: 100, w: 20, h: 150 }
    ],
    hides: [],
    npcs: [
      { x: 500, y: 200, name: 'Soekarno', r: 16, color: '#fbbf24',
        dialog: ['Apa maksud kalian datang tengah malam seperti ini?', 'Proklamasi harus melalui prosedur yang benar.', '...Baik. Kami ikut. Tapi tanggung jawab ada di tangan kalian.'] },
      { x: 600, y: 280, name: 'Hatta', r: 15, color: '#94a3b8',
        dialog: ['Tindakan ini sangat berisiko.', 'Jepang masih punya kekuatan di sini.', 'Semoga keputusan ini tidak salah.'] }
    ],
    items: [
      { x: 200, y: 150, id: 'lampu_minyak', name: 'Lampu Minyak', icon: '🪔', taken: false }
    ],
    patrols: [],
    exits: [
      { x: 700, y: 350, w: 55, h: 55, to: 4, label: 'Ke Rengasdengklok →', requireFlag: 'talked_soekarno' }
    ],
    alarmEnabled: false
  },

  // 4 - Rengasdengklok Halaman
  {
    id: 'rengas_halaman',
    title: 'Rengasdengklok · Siang 16 Agustus',
    objective: 'Jelajahi area. Bicara dengan penjaga PETA, lalu masuk rumah.',
    bg: '#121a10',
    spawn: { x: 80, y: 300 },
    walls: [
      { x: 400, y: 50, w: 30, h: 200 }, { x: 550, y: 200, w: 200, h: 25 }
    ],
    hides: [
      { x: 150, y: 80, w: 70, h: 50 }, { x: 280, y: 300, w: 70, h: 50 }
    ],
    npcs: [
      { x: 300, y: 180, name: 'Shodanco Singgih', r: 14, color: '#4ade80',
        dialog: ['Kami jaga sekeliling. Bung Karno aman di sini.', 'Pemuda di dalam sedang berdebat dengan beliau.', 'Masuk saja.'] }
    ],
    items: [
      { x: 500, y: 100, id: 'bendera_kecil', name: 'Bendera Merah Putih', icon: '🚩', taken: false }
    ],
    patrols: [
      { x: 200, y: 150, r: 11, path: [[200,100],[200,380]], idx: 0, speed: 1.1, vision: 50 }
    ],
    exits: [
      { x: 650, y: 280, w: 50, h: 55, to: 5, label: 'Masuk Rumah →' }
    ],
    alarmEnabled: true
  },

  // 5 - Dalam Rumah Rengasdengklok
  {
    id: 'rengas_dalam',
    title: 'Rumah Rengasdengklok · Debat',
    objective: 'Desak Soekarno & Hatta. Tunggu kabar dari Jakarta.',
    bg: '#1a140e',
    spawn: { x: 100, y: 320 },
    walls: [
      { x: 0, y: 0, w: 800, h: 25 }, { x: 0, y: 455, w: 800, h: 25 },
      { x: 0, y: 0, w: 25, h: 480 }, { x: 775, y: 0, w: 25, h: 480 }
    ],
    hides: [],
    npcs: [
      { x: 400, y: 200, name: 'Soekarno', r: 16, color: '#fbbf24',
        dialog: ['Kami mengerti semangat kalian.', 'Tapi kemerdekaan butuh persiapan matang.', 'Tunggu dulu... ada kabar dari Jakarta.'] },
      { x: 520, y: 260, name: 'Hatta', r: 15, color: '#94a3b8',
        dialog: ['Achmad Soebardjo sedang menuju ke sini.', 'Beliau membawa jaminan.'] }
    ],
    items: [],
    patrols: [],
    exits: [
      { x: 700, y: 300, w: 50, h: 55, to: 6, label: 'Soebardjo Tiba →', requireFlag: 'talked_rengas' }
    ],
    alarmEnabled: false
  },

  // 6 - Kedatangan Soebardjo (Ending map)
  {
    id: 'soebardjo',
    title: 'Rengasdengklok · 17.30 WIB',
    objective: 'Dengarkan jaminan Achmad Soebardjo. Putuskan sikapmu.',
    bg: '#14100c',
    spawn: { x: 150, y: 300 },
    walls: [
      { x: 0, y: 0, w: 800, h: 25 }, { x: 0, y: 455, w: 800, h: 25 }
    ...
    hides: [],
    npcs: [
      { x: 400, y: 220, name: 'Achmad Soebardjo', r: 15, color: '#a78bfa',
        dialog: ['Saya jamin dengan nyawa saya.', 'Proklamasi akan dilaksanakan besok, 17 Agustus.', 'Kita kembali ke Jakarta. Susun naskah di rumah Laksamana Maeda.'] },
      { x: 550, y: 280, name: 'Wikana', r: 14, color: '#38bdf8',
        dialog: ['Ini kesepakatan terbaik yang bisa kita raih.', 'Semangat perjuangan tidak boleh padam.'] }
    ],
    items: [],
    patrols: [],
    exits: [
      { x: 680, y: 300, w: 60, h: 55, to: 'end', label: 'Kembali ke Jakarta →' }
    ],
    alarmEnabled: false
  }
];
...
function getMap() { return MAPS[game.mapId]; }
...
function tryInteract() {
  ...
  if (near.type === 'npc') {
    openDialog(near.ref);
    // set flags
    if (near.ref.name === 'Soekarno' && game.mapId === 3) game.flags.talked_soekarno = true;
    if (near.ref.name === 'Soekarno' && game.mapId === 5) game.flags.talked_rengas = true;
